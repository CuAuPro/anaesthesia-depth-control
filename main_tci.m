...
%Volume 4 is very small, so k1ep is smaller than ke0p (for propofol)
%k1ep = k1ep/1000;

%Convert to units with seconds
k10p = k10p/60;
k12p = k12p/60;
k13p = k13p/60;
k21p = k21p/60;
...
%Steady state concetration
%% References

BIS_REF = 50;
Uremi = CER_REF./C50R;
CEP_REF = BIStoContentration(BIS_REF, Uremi, gammaBIS, sigmaBIS, Emax).*C50P
...
%Volume 4 is very small, so k1ep is smaller than ke0p (for propofol).
%Refer to above mentioned article.
k14 = k1ep/10000;

out = sim('Patient_TCI.slx');
out_tci = out;
%%
%%
...
hold on;
plot(out.time./60, out.ce_est, 'r');
plot(out.time./60, out.CePrs, 'b');
legend('$ref$', '$x_{estTCI}$', '$x_{est}$', 'Interpreter', 'latex');
xlabel("$t [min]$", 'Interpreter', 'latex');
ylabel("$x_e [mg/ml]$", 'Interpreter', 'latex');
ylim([0, max(out.CePrs)*1.05]);
