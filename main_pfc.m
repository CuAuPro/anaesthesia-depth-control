...
dBIS_est = pade(exp(-BISdelay*s),4);
...
subplot(3,1,2);
% plot(time./60, xe_ref, 'k');
% hold on;
% plot(time./60, xe_refModel, 'r');
% plot(time./60, xe_val, 'b');
plot(out.time./60, out.ce_prop_ref, 'k');
hold on;
plot(out.time./60, out.ce_prop_refModel, 'r');
...
ylim([0, max(out.ce_prop_refModel)*1.05]);
